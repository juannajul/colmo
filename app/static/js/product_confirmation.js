		...
		.then((confirmation) => {
			...
			confirmations.forEach((conf) => {
				...
				if (confirmationStatus === true) {
					let confirmationBtn = document.getElementById(
						`product-btn-confirmation_${conf.id}`
					);
					confirmationBtn.style.display = "none";
					let finishBtn = document.getElementById(
						`product-btn-cancelation_${conf.id}`
					);
					finishBtn.style.display = "none";
				}
				if (finishedStatus === true) {
					let confirmationBtn2 = document.getElementById(
						`product-btn-confirmation_${conf.id}`
					);
					confirmationBtn2.style.display = "none";
					let finishBtn2 = document.getElementById(
						`product-btn-cancelation_${conf.id}`
					);
		...
		.then((data) => {
			...
			confirmation_list.forEach((productsConfirmation) => {
				const confirmationContainer = document.querySelector(
					".item-confirmation-container"
				);

				confirmationContainer.innerHTML = productsConfirmation
					.map(function (conf) {
						let conf_id = conf.id;
              ...
              <div class="product-confirmation-container">
                  ...
                  <div id="product-confirmation-products-container" class="product-confirmation-products-container-${conf.id}"></div>
		...
		conf_id_list.forEach((id) => {
			fetch(`/api/sold_products/${id}/product_confirmation/`)
				...
				.then((data) => {
					...
					const soldProductConfirmation = document.querySelector(
						`.product-confirmation-products-container-${id}`
					);
					soldProductConfirmation.innerHTML = products
			...
			fetch(`/api/sold_products/${confirmationId}/product_confirmation/`)
				...
				.then((data) => {
					...
					products.forEach((product) => {
						let size_qty = document.getElementById(
							`confirmation-product-size-qty_${product.product_size}`
						).innerHTML;
								...
								if (newQty < 0) {
									...
									fetch(`/api/product_sizes/${sizeId}/`, {
										...
										.then((data) => {
											console.log(data);
											fetch(
												`/api/sold_products_confirmation/${confirmationId}/`,
												{
													method: "PATCH",
													headers: {
														"Content-Type": "application/json",
